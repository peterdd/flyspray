...
while ($row = $db->FetchRow($milestones)) {
	...
	$tasks = $db->Query('
    		SELECT task_id, item_summary, detailed_desc, task_severity, task_priority, task_type, mark_private, opened_by, content, task_token, t.project_id, estimated_effort
		FROM {tasks} t
